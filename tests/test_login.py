...
class TestsPageWiews:
    ...
    def test_profile_login_ckick_open_profile(self, driver, login):     #Проверка открытия страницы профиля
    ...
    def test_constructor_across_profile_profile_open_constructor(self, driver, login):      #Проверка перехода на страницу конструктора (главная) из профиля
        TestsPageWiews.test_profile_login_ckick_open_profile(self, driver, login)
